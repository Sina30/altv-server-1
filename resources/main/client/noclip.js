...
const KeyInfos = {
    ...
    DownKey: "~INPUT_VEH_MOVE_DOWN_ONLY~",
    UpKey: "~INPUT_VEH_MOVE_UP_ONLY~",
...
alt.everyTick(() => {
    if (NoclipActive) {
        ...
        if (alt.Player.local.vehicle) game.disableControlAction(0, KeyControls.RadioWheel, true);
        ...
        if (game.updateOnscreenKeyboard() !== 0 && alt.isGameFocused()) {
            ...
            if (game.isDisabledControlPressed(0, KeyControls.MoveRightOnly)) {
                xoff = 2;
            }
            if (game.isDisabledControlPressed(0, KeyControls.MoveLeftOnly)) {
                xoff = -2;
            }
