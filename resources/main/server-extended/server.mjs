...
alt.on('playerConnect', (player) => {
	if (markersToLoad.length >= 1) {
		for(const element of markersToLoad) {
			...
			alt.emitClient(
				...
				{x: 0, y: 0, z: 0},
				element.markerScale,
				element.exitMarkerColor,
				element.enterMarkerColor,
				false,
				element.markerRange,
				undefined
			);
...
alt.on('entityEnterColshape', (colshape, entity) => {
	...
	if (colshape.isPlayerOnly && entity instanceof alt.Player) {
		alt.log('Called server-side event.');
...
setInterval(() => {
	...
	alt.log(`===> Extended: Cleaned up ${callbacksCleanedUp}`);
}, 5000);